...
class GameEngine:
    ...
    def play_draft(self):   
        ...
        if any(not self.is_valid_word(word) for word in all_words):
            self.logs.append("Not all formed words are valid")
            return
        ...
        self.logs.append("Player played" if self.gamestate.p1_to_play else "AI played")
    ...
    def ai_handle_turn(self):
        ...
        sorted_option_idx = [x for x,y in sorted(enumerate(points), key = lambda x: (points[x[0]], sim_points[x[0]], all_words[x[0]]))]
    ...
    def swap_draft(self, swap_all = False):
        ...
        if is_swap_allowed:
            ...
            self.pass_turn()
            if tiles_to_swap == 0:
                self.logs.append(("Player " if self.gamestate.p1_to_play else "AI ") + "passed turn ({})".format(self.exchanges))
            else:
                self.logs.append(("Player " if self.gamestate.p1_to_play else "AI ") + "swapped tiles ({})".format(self.exchanges))
            self.check_game_end()
            return True
        else:
            if tiles_to_swap > 0:
                self.logs.append(("Player " if self.gamestate.p1_to_play else "AI ") + "swap failed, not enough tiles ({})".format(tiles_to_swap))
                return False
            else:
                self.pass_turn()
                self.logs.append(("Player " if self.gamestate.p1_to_play else "AI ") + "passed turn ({})".format(self.exchanges))    
