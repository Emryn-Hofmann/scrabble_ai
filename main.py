...
from Game import scrabble_game
import pygame as p


gamestate = scrabble_game.GameState(
    "Data/multipliers.txt", "Data/lang/en/tiles.txt"
)
gameengine = scrabble_game.GameEngine(
    gamestate, "Data/lang/en/850_ogdens_words.txt"
)
...
screen = p.display.set_mode(
    (scrabble_renderer.WINDOW_WIDTH, scrabble_renderer.WINDOW_HEIGHT)
)
...
while running:
                ...
                if not renderer.swap_mode:
                    if event.button == 1:
                        ...
                        if board_is_col or rack_is_col:
                            if board_is_col:
                                cell = gamestate.board.board[board_tile[0]][
                                    board_tile[1]
                                ]
                    ...
                    if event.button == 1:
                        if dragging:
                            if not drag_mode_board:
                                board_is_col, board_tile = renderer.board_col_detect(
                                    *event.pos
                                )
                                if not board_is_col:
                                ...
                                if board_is_col:
                                    board_is_col, board_tile = (
                                        renderer.board_col_detect(*event.pos)
                                    )
                                    cell = gamestate.board.board[board_tile[0]][
                                        board_tile[1]
                                    ]
                                    if cell.tile is None:
                                        ...
                                        old_cell = gamestate.board.board[
                                            drag_old_pos[0]
                                        ][drag_old_pos[1]]
                                        old_cell.tile = dragging_tile
                                        dragging = False
                                else:
                                    old_cell = gamestate.board.board[drag_old_pos[0]][
                                        drag_old_pos[1]
                                    ]
                                    old_cell.tile = dragging_tile
                                    dragging = False
    ...
    if dragging:
        ...
        game_surf.blit(
            drag_surf,
            (
                event.pos[0] - scrabble_renderer.SQ_SIZE // 2,
                event.pos[1] - scrabble_renderer.SQ_SIZE // 2,
            ),
        )
